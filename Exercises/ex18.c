...
int *bubble_sort(int *numbers, int count, compare_cb cmp)
{
    ...
    int i = 0;
    int j = 0;
    int *target = malloc(count*sizeof(int)); // allocating enough space for an array

    if (!target) die("Memory error.");

    memcpy(target, numbers, count*sizeof(int)); // copying the array locally
